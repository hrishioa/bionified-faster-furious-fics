...
export default function Kudos() {
  const kudosCount = useAppStoreSelector(
    (state) => state.work.workInfo?.kudos || 0,
  );
  const lottieRef = useRef(null as LottieRefCurrentProps | null);
  const workId = useAppStoreSelector((state) => state.work.workInfo?.id);
  const authenticityToken = useAppStoreSelector(
    (state) => state.work.workInfo?.authenticityToken,
  );
      ...
      window
        ...
        .then(async (res) => {
          if (res.status === 200) {
            ...
            if (data.success) dispatch(addKudos());
          }
        });
    }
  }
  ...
  return (
    <div className="kudos-icon" onClick={kudosLeft}>
      ...
      />
      <div className="kudos-count">{kudosCount}</div>
    </div>
  );
}
